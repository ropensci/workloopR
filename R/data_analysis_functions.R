...
#' Analyze work loop file to compute work and power output
#'
...
analyze_workloop<-function(x,
  ...
  work<-lapply(x_by_cycle,function(x)-trapezoidal_integration(x$Position/1000,x$Force/1000))
  ...
  instant_power<-mapply(function(x,v)x$Force*v/1000,x_by_cycle,filt_velocity,SIMPLIFY=FALSE)
...
#' Compute timing of twitch kinetics
#'
...
isometric_timing<-function(x,
  ...
  # check that rising_perc1 and ending_prop are numeric between 0 and 100
