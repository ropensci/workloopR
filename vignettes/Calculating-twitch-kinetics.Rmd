...
The `workloopR` package also provides a function that can calculate the timing and magnitude of force duirng isometric experiments (twitch, tetanus) via the `isometric_timing()` function.
...
The `isometric_timing()` function can also work on `tetanus` objects that have been imported via `read_ddf()`. Should a `tetanus` object be used, the set points for relaxing are automatically set to `relaxing = c()`, which excludes this argument from producing anything. Instead, the timing & magnitude of force at stimulation, peak force, and specifed points of the rising phase are returned. The idea of 'relaxation' is simply ignored. 
